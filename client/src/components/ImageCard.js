  ...
  return (
    ...
    <div className=" grid grid-cols-3">
      {data.imageCards.map((imageCards) => (
          <div
            key={imageCards._id}
            className="bg-gray-200 rounded shadow-lg p-3 m-2 font-bold text-green-700 text-2xl text-center"
          >
            <div className=" m-5">
              {imageCards.title} by {imageCards.imageAuthor}
              </div>
            <img className="w-full mb-3" src={imageCards.image} alt="" />
            <div className="border rounded border-neutral-500 mb-10 text-md md:mt-0">
              <p className="text-green-700 text-lg text-start py-6">{imageCards.description}</p>
